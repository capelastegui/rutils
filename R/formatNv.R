# Methods for clear display of numeric vectors Useful for linear models, glms, and others
...
printGlm <- function(x, digits = max(3L, getOption("digits") - 3L), ...) {
    ...
    if (length(coef(x))) {
        ...
        print.default(format(aaply(x$coefficients, 1, format, digits = digits), justify = "right"), print.gap = 2, 
            quote = FALSE)
...
printGlmSum <- function(y, digits = max(3L, getOption("digits") - 3L), symbolic.cor = x$symbolic.cor, signif.stars = getOption("show.signif.stars"), 
    ...) {
    if ("glm" %in% class(y)) {
        x <- summary(y)
    } else {
    ...
    if (x$df.residual > 5) {
        x$deviance.resid <- setNames(quantile(x$deviance.resid, na.rm = TRUE), c("Min", "1Q", "Median", "3Q", 
            "Max"))
    }
    ...
    if (length(x$aliased) == 0L) {
        ...
        printCoefmatNV(coefs, digits = digits, signif.stars = signif.stars, na.print = "NA", ...)
    }
    cat("\n(Dispersion parameter for ", x$family$family, " family taken to be ", format(x$dispersion), ")\n\n", 
        apply(cbind(paste(format(c("Null", "Residual"), justify = "right"), "deviance:"), format(unlist(x[c("null.deviance", 
            "deviance")]), digits = max(5L, digits + 1L)), " on", format(unlist(x[c("df.null", "df.residual")])), 
...
printCoefmatNV <- function(x, digits = max(3L, getOption("digits") - 2L), signif.stars = getOption("show.signif.stars"), 
    signif.legend = signif.stars, dig.tst = max(1L, min(5L, digits - 1L)), cs.ind = 1:k, tst.ind = k + 1, zap.ind = integer(), 
    P.values = NULL, has.Pvalue = nc >= 4 && substr(colnames(x)[nc], 1, 3) == "Pr(", eps.Pvalue = .Machine$double.eps, 
    na.print = "NA", ...) {
    ...
    if (length(cs.ind)) {
        ...
        if (any(ia <- is.finite(acs))) {
            ...
            Cf[, cs.ind] <- formatNv(round(coef.se, max(1L, digits - digmin)), digits = digits)
        }
    }
    if (length(tst.ind)) 
        Cf[, tst.ind] <- formatNv(round(xm[, tst.ind], digits = dig.tst), digits = digits)
    ...
    if (length(not.both.0 <- which(x0 & !is.na(x0)))) {
        Cf[okP][not.both.0] <- formatNv(xm[okP][not.both.0], digits = max(1L, digits - 1L))
    }
    if (any(ina)) 
        Cf[ina] <- na.print
    if (P.values) {
        ...
        if (any(okP <- ok[, nc])) {
            ...
            if (signif.stars) {
                Signif <- symnum(pv, corr = FALSE, na = FALSE, cutpoints = c(0, 0.001, 0.01, 0.05, 0.1, 1), 
                  symbols = c("***", "**", "*", ".", " "))
...
printAovSum <- function(object, intercept = FALSE, split, expand.split = TRUE, keep.zero.df = TRUE, ...) {
        ...
        for (i in unique(asgn)) {
            ...
            if (any(sp)) {
                if (sum(sp) > 1) {
                  old <- split[f[sp]]
                  nn <- setNames(nm = f[sp])
                  marg <- lapply(nn, function(x) df.names[asgn == (match(x, names) - 1L)])
                  term.coefs <- strsplit(df.names[asgn == i], ":", fixed = TRUE)
                  ttc <- sapply(term.coefs, function(x) x[sp])
                  rownames(ttc) <- nn
                  splitnames <- setNames(nm = apply(expand.grid(lapply(old, names)), 1L, function(x) paste(x, 
                    collapse = ".")))
                  ...
                  split[[names[i + 1L]]] <- new[sapply(new, function(x) length(x) > 0L)]
                } else {
                  old <- split[[f[sp]]]
    ...
    if (!is.null(effects) && !missing(split)) {
        ...
        if (!is.null(Terms <- object$terms)) {
            ...
            if (!all(ns %in% nmeffect)) {
                ...
                stop(sprintf(ngettext(na, "unknown name %s in the 'split' list", "unknown names %s in the 'split' list"), 
                  paste(sQuote(ns[na]), collapse = ", ")), domain = NA)
            }
        }
        if (expand.split) {
