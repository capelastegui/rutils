...
ggROCR.roc <- function(perf, title = NULL) {
    ...
    p <- ggplot(perf.df) + aes(x = x, y = y) + geom_line() + geom_point() + scale_x_continuous(limits = c(0, 
        1)) + scale_y_continuous(limits = c(0, 1)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
        theme(plot.title = element_text(size = 12)) + labs(x = attr(perf.df, "XLabel"), y = attr(perf.df, "YLabel"), 
        title = title)
    if (!is.null(perf.df$.id)) {
        p <- p + aes(color = .id)
    } else {
...
ggROCR.prrc <- function(perf, title = NULL) {
    ...
    p <- ggplot(perf.df) + aes(x = x, y = y) + geom_line() + geom_point() + scale_y_continuous(limits = c(0, 
        1)) + scale_x_continuous(limits = c(0, 1)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
        theme(plot.title = element_text(size = 12)) + labs(x = attr(perf.df, "XLabel"), y = attr(perf.df, "YLabel"), 
        title = title)
    if (!is.null(perf.df$.id)) {
        p <- p + aes(color = .id)
...
ggROCR.f <- function(perf, title = NULL) {
    ...
    p <- ggplot(perf.df) + aes(x = x, y = y) + geom_line() + geom_point(alpha = 0.7) + scale_y_continuous(limits = c(0, 
        1)) + scale_x_continuous(limits = c(0, 1)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
        theme(plot.title = element_text(size = 12)) + labs(x = attr(perf.df, "XLabel"), y = "F-measure", title = title)
    
    if (!is.null(perf.df$.id)) {
        p <- p + aes(color = .id) + geom_hline(aes(yintercept = y.max, color = .id), alpha = 0.4)
    }
    p
}
...
ggROCR.generic <- function(perf, title = NULL) {
    ...
    p <- ggplot(perf.df) + aes(x = x, y = y, color = alpha) + geom_line() + geom_point() + labs(x = attr(perf.df, 
        "XLabel"), y = attr(perf.df, "YLabel"), title = title)
    if (!is.null(perf.df$.id)) {
        p <- p + aes(color = .id)
    }
    p
...
ggROCR.generic.list <- function(perfList, title = NULL) {
    ...
    ggplot(perf.df) + aes(x = x, y = y, color = .id) + geom_line() + geom_point() + labs(x = attr(perf.df, 
        "XLabel"), y = attr(perf.df, "YLabel"), title = title)
}
...
# pred <- predict (.glm, .glm$model, type='response') pred.rocr <- prediction(pred, .glm$model$target)
...
prediction.custom <- function(labels, .fun) {
    ...
    myfun <- .fun
    predictions <- llply(labels, myfun, .fun = function(l1, myfun) {
        laply(l1, .fun = myfun)
    })
...
prediction.fixed <- function(labels) {
    prediction.custom(labels, .fun = function(l1) {
        0.5
    })
}
...
#### Takes as input a prediction table, a glm, a model threshold and a window size pred.match.df will have as
#### columns: interval (date intervals of size=windowSize), variable - whether a measure corresponds to data
#### labels ('target') or predictions ('pred.glm') value - whether a data point has an event (TRUE) or not.
...
# usage: getPredMatchDf(predTable=get.nested(predTables,c(1,1,1)), .glm=get.nested(glmList,c(1,1,1)),
# threshold=0.5)
...
getPredMatchPlot <- function(predMatchDf, size = 2, vjitter = 1, alpha = 0.7, colorMatches = TRUE, legend.position = "top") {
    ...
    pred.match <- ggplot(predMatchDf) + aes(x = interval, y = events) + geom_point(position = position_jitter(0, 
        vjitter), size = size, alpha = alpha) + facet_grid(variable ~ value) + theme(legend.position = "top") + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme(plot.title = element_text(size = 12)) + 
        labs(x = "Date", y = "Event Count", title = paste("Prediction Match; th=", threshold))
    if (colorMatches) {
        pred.match <- pred.match + aes(color = match) + theme(legend.position = legend.position)
    }
    return(pred.match)
}


# TODO: how to make a prediction.COIN with random outcomes?  how to give it a customizable threshold that
# is compatible with ROCR threshold plots? 
